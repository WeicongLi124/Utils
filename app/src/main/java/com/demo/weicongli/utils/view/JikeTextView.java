...
public class JikeTextView extends View {
    ...
    private final int TEXT_SIZE = 60; //文字高度
    private final int centerY = 116;

    private boolean isAdd = true; //是否为数量增加
    private boolean isInit = false; //是否初始化是设置值
    private String newAnimateText; //新值，需要动画的Text
    private String oldAnimateText; //旧值，需要动画的Text
    ...
    {
        mStableTextPaint.setTextScaleX(TEXT_SIZE);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawText(oldAnimateText, mStableTextWidth, oldAnimateY, mOldTextPaint);
    }
    ...
    private void setAnimateText() {
        ...
        if (isAdd) {
            if (isChangeTwo(true)) {
                ...
                oldAnimateText = String.valueOf(curCount - 1).substring(text.length() - 2);
                mStableTextWidth = mStableTextPaint.measureText(text, 0, text.length() - 1);
            } else {
                newAnimateText = text.substring(text.length() - 1);
                oldAnimateText = String.valueOf(curCount - 1).substring(text.length() - 1);
                mStableTextWidth = mStableTextPaint.measureText(text, 0, text.length() - 1);
            }
        } else {
    ...
    public void decentCurCount() {
        ...
        mValueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                oldAnimateY = centerY + (HEIGHT - centerY) * temp / 100f;
                newAlpha = temp / 100f;
                oldAlpha = 1 - temp / 100f;
                invalidate();
            }
